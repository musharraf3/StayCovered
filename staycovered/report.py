"""HTML report renderer — the StayCovered 'glass box' UI.
...
def render(result: ReviewResult, request: dict, by_id: dict[str, Chunk]) -> str:
    ...
    clock = ""
    if d:
        label = ("DEADLINE PASSED — reconsideration window running" if d.breached
                 else f"{d.hours_remaining / 24:.0f} days remaining")
        clock = (f'<span class="clock{" breach" if d.breached else ""}">'
                 f'{_e(d.track)} notice · respond by {_e(d.due_at)} · {label}</span>')

    parts = [f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<title>StayCovered — {_e(result.request_id)}</title><style>{CSS}</style></head><body><div class="wrap">
<header><h1>StayCovered</h1><span class="tag">renewal preparation ·
{_e(request['member']['state'])} · {_e(result.model)} · arm: {_e(result.arm)}</span></header>
<div style="margin:6px 0 0">{clock}</div>
<div class="banner {result.decision.value}">{BANNER[result.decision.value]}</div>
<div class="card"><h2>The notice</h2>
<p><b>{_e(request['notice']['title'])}</b> — {_e(request['member']['name'])},
case {_e(request['member']['member_id'])}</p>
<p style="color:var(--sub);font-size:13.5px;margin-top:6px">{_e(request.get('situation', ''))}</p></div>"""]
    ...
    if result.findings:
        ...
        parts.append(f'<div class="card"><h2>Renewal screening <span class="badge code">'
    ...
    parts.append(f'<div class="card"><h2>Case notes (for the helper)</h2><div class="memo">{_e(result.memo)}</div>'
                 + (f'<div class="impact"><b>Letter to the member (plain language):</b> '
                    f'{_e(result.member_impact)}</div>' if result.member_impact else "")
                 + '</div>')

    if result.missing_documents:
        items = "".join(f"<li>{_e(m)}</li>" for m in result.missing_documents)
        parts.append(f'<div class="card"><h2>Proof still needed <span class="badge code">'
    ...
    if result.escalate:
        parts.append(f'<div class="escalate">⚠ Free human help recommended: {_e(result.escalate_reason)}</div>')
...
The notice clock, screening, hours math, citation verification, and policy overrides are
deterministic code — inspect them in the repo. StayCovered prepares people; it files
nothing, decides nothing, and is not legal advice. All data on this page is synthetic.</footer>
</div></body></html>""")
    return "".join(parts)
