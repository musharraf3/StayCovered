"""Three-arm eval harness for StayCovered.
...
from staycovered.corpus import chunk_index, load_corpus  # noqa: E402
from staycovered.grounding import apply_guard  # noqa: E402
from staycovered.models import ReviewResult  # noqa: E402
from staycovered.pipeline import apply_policy  # noqa: E402

REQS = ROOT / "examples" / "cases"
...
def get_result(request: dict, arm: str, skip_cached: bool) -> ReviewResult:
    ...
    from staycovered.pipeline import run_review
...
def main() -> int:
    ...
    ap.add_argument("--teacher-subset", default="case_02,case_05,case_09",
                    help="Budget control: the expensive teacher arm runs only these cases "
                         "as a spot-check of the ceiling. Pass '' to run the full suite.")
    args = ap.parse_args()
    arms = [a.strip() for a in args.arms.split(",")]
    subset = {c.strip() for c in args.teacher_subset.split(",") if c.strip()}

    all_cases = sorted(REQS.glob("*.json"))
    table_rows = []

    for arm in arms:
        cases = ([p for p in all_cases if p.stem in subset]
                 if arm == "teacher+skill" and subset else all_cases)
        ...
        label = f"{arm} (spot-check, {len(cases)} cases)" if cases is not all_cases else arm
        table_rows.append(f"| {label} | {model_id} | {row['dec']} | {row['mm']} | {row['ground']} "
                          f"| {row['esc']} | {row['tools']} | {row['fk']} | {row['cost']} |")
        print(f"== {label}: {row}\n")

    md = f"""# StayCovered eval results

Generated {date.today().isoformat()} · {len(all_cases)} synthetic renewal cases · all numbers from
